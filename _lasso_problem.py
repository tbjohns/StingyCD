...
class LassoProblem:
  ...
  def reset_A_and_b(self, A, b):
    ...
    if (sparse.issparse(A)):
      ...
      lib.CD_set_A_sparse(self.blitzObj, dataArg, indicesArg, indptrArg, ctypes.c_uint(A.nnz), ctypes.c_uint(n), ctypes.c_uint(d))

    else:

      matrixArg = double_pointer(A)
      lib.CD_set_A_dense(self.blitzObj, matrixArg, ctypes.c_int(n), ctypes.c_int(d), A.ctypes.strides)
  ...
  def solve(self, lam, x0=None, log_dir=""):
    ...
    logArg = ctypes.c_char_p(log_dir)
